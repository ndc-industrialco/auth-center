...
### 10.5 ค้นหาเมลของผู้ใช้เพื่อทำ CRM

ระบบ CRM สามารถให้ผู้ใช้กด Sync แล้วส่งเงื่อนไขค้นหาเข้ามาที่ Auth Center ได้ โดย Auth Center จะใช้ identity จาก Auth Center JWT เพื่อเลือก mailbox ของผู้ใช้คนนั้นเอง และเรียก Microsoft Graph ให้ ไม่ควรให้ CRM ส่ง `userId`, mailbox อื่น หรือ raw Graph URL เข้ามากำหนดเอง

```text
ผู้ใช้ login CRM ผ่าน Auth Center
        |
        | กด Sync Mail พร้อม folder/เงื่อนไข
        v
CRM เรียก POST /api/auth/consumer/mail/search
        |
        | Bearer Auth Center JWT
        v
Auth Center ตรวจ app, session, Entra-linked user และ mailbox
        |
        v
Microsoft Graph /users/{user-upn}/mailFolders/{folder}/messages
        |
        v
Auth Center ส่งผลแบบ batch กลับให้ CRM บันทึกเอง
```
...
```json
{
  ...
  "toDate": "2026-07-15T23:59:59Z",
  "limit": 100
}
```

รองรับ folder ที่กำหนดไว้เท่านั้น:

```text
inbox, sentitems, archive, deleteditems, drafts, junkemail
```
...
| Field | รายละเอียด |
|---|---|
| `appId` | Application ID ที่ตรงกับ JWT audience |
| `folder` | Folder ที่ต้องการค้นหา ค่าเริ่มต้นคือ `inbox` |
| `fromEmail` | ค้นหาจาก email ของผู้ส่ง |
| `keyword` | ค้นหาในความสามารถ full-text ของ Graph เช่น sender, subject และ body |
| `fromDate` / `toDate` | กรองจาก `receivedDateTime` แบบ ISO 8601 พร้อม timezone |
| `limit` | จำนวนสูงสุดที่ต้องการต่อการ sync ค่าสูงสุด `1000` |
...
CRM ควรใช้ `id` หรือ `internetMessageId` เป็น key สำหรับ upsert และเก็บ `conversationId` เพื่อจัดกลุ่ม thread ไม่ควรใช้ subject หรือ email เป็น primary key

ข้อกำหนดสำคัญ:

- ผู้ใช้ต้อง login ด้วย Entra และมี `m365Linked = true`.
- Endpoint นี้ค้น mailbox ของผู้ใช้จาก JWT เท่านั้น ไม่รองรับการเลือก mailbox ของบุคคลอื่น.
- Auth Center ไม่เก็บ message หรือ attachment ลง database; CRM เป็นเจ้าของข้อมูลที่ sync แล้ว.
- ให้ CRM เรียกเป็น batch และจัดการการบันทึกซ้ำด้วย Graph message ID.
- Graph `$search` เป็น full-text search ไม่ใช่ SQL `contains`; ผลลัพธ์และ tokenization ให้ยึดตาม Graph.
- การ sync ปริมาณมากควรแบ่งตาม folder/date range และตรวจ `hasMore`.

การตั้งค่า Production ต้องให้ Azure application ของ Auth Center มี Microsoft Graph Application Permission ที่เหมาะสม เช่น `Mail.Read` และจำกัด mailbox ด้วย Exchange RBAC for Applications ก่อนเปิดใช้งานจริง ไม่ควรใช้สิทธิ์อ่านทุก mailbox โดยไม่มี scope

#### ตัวอย่าง service ฝั่ง CRM

```ts
export async function searchOwnMailbox(input: {
  authCenterAccessToken: string;
  folder: string;
  fromEmail?: string;
  keyword?: string;
  fromDate?: string;
  ...
  const response = await fetch(
    `${process.env.AUTH_CENTER_URL}/api/auth/consumer/mail/search`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${input.authCenterAccessToken}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(`Auth Center mail search failed: ${response.status}`);
  }
...
อย่าเรียก endpoint นี้จาก browser โดยตรงถ้าจะส่งต่อ access token; ให้เรียกจาก server route/action ของ CRM และไม่ log token หรือเนื้อหา mail

### 10.6 ข้อควรระวังด้านความปลอดภัยและความเสถียร
...
## 12. Logout และ session registry
...
## 16. Definition of Done สำหรับระบบใหม่
...
- ถ้ามี CRM mail sync: เรียก mail search ผ่าน Auth Center โดยใช้ folder/criteria ที่ allowlist และบันทึกผลลง CRM database เท่านั้น.
