...
export function RevokeButton({ grantId, role }: { grantId: string; role: string }) {
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] p-6 w-full max-w-sm">
          <Dialog.Title className="text-lg font-semibold text-slate-800 mb-2">Revoke Role Grant</Dialog.Title>
          <Dialog.Description className="text-sm text-slate-600 mb-6">
            Revoke <span className="font-mono font-semibold text-slate-800">{role}</span>? This action cannot be undone.
          </Dialog.Description>
